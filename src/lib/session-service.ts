...
function dto(session: StoredSession): PublicSessionDto {
  const states = session.record.chunks.map((chunk) => chunk.continuityState)
  const continuitySummary: ContinuityState = states.includes('confirmed-gap')
    ? 'confirmed-gap'
    : states.includes('continuity-unknown')
      ? 'continuity-unknown'
      : 'observed-continuous'
  return { protocolVersion: 1, sessionId: session.record.sessionId, state: session.record.state, revision: session.revision, chunkCount: session.record.chunks.length, continuitySummary }
}
